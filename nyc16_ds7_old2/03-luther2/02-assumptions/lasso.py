...
data = maker(N,20,0.7)    #generate the data assuming that 70% of variables matter
y = data[1]    #pull out the dependent variable
X = data[0].T    #pull out the independent variable

#X = pd.DataFrame(X)
#X['y'] = y
#X.to_csv('/Users/seniordatascientist/Desktop/Metis Codes/Data/Lasso_practice_data.csv')
X = pd.read_csv('/Users/seniordatascientist/Desktop/Metis Codes/Data/Lasso_practice_data.csv')
y = X['y']
X = X.drop('y',1)
X = np.array(X)
y = np.array(y)
